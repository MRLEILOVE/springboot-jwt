...
/**
 * Date 类型时间反序列化，可接受前端传过来的 yyyy-MM-dd HH:mm:ss 和 yyyy-MM-dd 两种格式时间， 当传 yyyy-MM-dd 格式时，时分秒填充0
 *
 * @author leiguoqing
 * @date 2020-08-08 17:07:30
 */
public class DateDeserializer extends JsonDeserializer<Date> {
    ...
    public Date deserialize(JsonParser jp, DeserializationContext context) throws IOException {
        ...
        if (isDateFormat) {
            ...
            return DateUtils.Date.from(localDate);
        } else {
            return DateUtils.Date.from(text, "yyyy-MM-dd HH:mm:ss");
        }
    }
}
